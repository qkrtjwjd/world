...
/// 지지직— 하고 갈라진 소리를 뱉는다."
///
/// ⚠ **라디오가 스스로 재생되는 것은 이 씬이 유일하다.** 첫 재생만 예외라는 것을 잊지 말 것.
///
/// 2026-08-30 — 「이후로는 [라디오] 선택지로 호출된다(Radio_Yu.yarn / RadioManager)」는
///   구 설계였다. E-52 가 그 방식을 폐기했고 셋 다 삭제했다. 이후의 유의 반응은
///   <b>대비 오브젝트를 조사하면 결과 뒤에 한 줄</b>이 붙는 형태이며, 대비 노드 안에서
///   <c>$라디오소지</c> 로 조건 분기한다(F-8-4). 선택지도 별도 노드도 매니저도 두지 않는다.
///   구 D이관-3절 규약은 그 선택지 방식을 전제한 것이므로 여기서는 따르지 않는다.
///
